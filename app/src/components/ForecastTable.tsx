...
export default function ForecastTable({ days, hourly }: ForecastTableProps) {
          ...
          {days.map((day) => {
            ...
            return (
              <Fragment key={day.date}>
                <tr
                  onClick={() => hourly && setExpandedDate(expandedDate === day.date ? null : day.date)}
                  className={cn(
                    "border-b border-border/50 last:border-b-0 transition-colors",
                    hourly && "cursor-pointer",
                    isPowder
                      ? "bg-accent-orange/5 hover:bg-accent-orange/10"
                      : "hover:bg-bg-elevated/50",
                    expandedDate === day.date && "bg-bg-elevated/30"
                  )}
                >
                ...
                {expandedDate === day.date && hourly && (
                  <tr>
                    <td colSpan={8} className="px-4 py-3 bg-bg-primary/50">
                      <DayDetailPanel date={day.date} hourly={hourly} />
                    </td>
                  </tr>
                )}
              </Fragment>
